...
if(isset($_POST['tambah']))
{
		...
		$targetDir = "/Applications/XAMPP/xamppfiles/htdocs/Admin/vertical/assets/images/products/";
		...
		if ($uploadOk == 1) {
			if (move_uploaded_file($_FILES["Gambar"]["tmp_name"], $targetFile)) {
				...
				$query = mysqli_query($koneksi,"INSERT INTO produk (ProdukID, NamaProduk, KategoriID, HargaSatuan, Stok, Deskripsi, GambarProduk)			
				...
				if ($run) {
					echo "Data tersimpan. File ". htmlspecialchars(basename($_FILES["Gambar"]["name"])) . " telah di-upload.";
					echo "<script>setTimeout(function(){ window.location.href = 'app-product-list.php'; }, 3000);</script>";
				} else {
